...
package com.gio.subzero.view.action.mediaplayer;
...
public final class MediaPlayerActions {
    ...
    public MediaPlayerActions(MediaPlayer mediaPlayer) {
        ...
        videoSnapshotAction     = new SnapshotAction(Resource.resource("menu.video.item.snapshot"), mediaPlayer);
    }

    private List<Action> newPlaybackSpeedActions(MediaPlayer mediaPlayer) {
        List<Action> actions = new ArrayList<>();
        actions.add(new RateAction(Resource.resource("menu.playback.item.speed.item.x4"    ), mediaPlayer, 4f));
        actions.add(new RateAction(Resource.resource("menu.playback.item.speed.item.x2"    ), mediaPlayer, 2f));
        actions.add(new RateAction(Resource.resource("menu.playback.item.speed.item.normal"), mediaPlayer, 1f));
        actions.add(new RateAction(Resource.resource("menu.playback.item.speed.item./2"    ), mediaPlayer, 0.5f));
        actions.add(new RateAction(Resource.resource("menu.playback.item.speed.item./4"    ), mediaPlayer, 0.25f));
        return ImmutableList.copyOf(actions);
    }

    private List<Action> newPlaybackSkipActions(MediaPlayer mediaPlayer) {
        List<Action> actions = new ArrayList<>();
        actions.add(new SkipAction(Resource.resource("menu.playback.item.skipForward" ), mediaPlayer,  10000));
        actions.add(new SkipAction(Resource.resource("menu.playback.item.skipBackward"), mediaPlayer, -10000));
        return ImmutableList.copyOf(actions);
    }

    private List<Action> newPlaybackChapterActions(MediaPlayer mediaPlayer) {
        List<Action> actions = new ArrayList<>();
        actions.add(new PreviousChapterAction(Resource.resource("menu.playback.item.previousChapter"), mediaPlayer));
        actions.add(new NextChapterAction    (Resource.resource("menu.playback.item.nextChapter"    ), mediaPlayer));
        return ImmutableList.copyOf(actions);
    }
    ...
    private List<Action> newAudioStereoModeActions(MediaPlayer mediaPlayer) {
        List<Action> actions = new ArrayList<>();
        actions.add(new StereoModeAction(Resource.resource("menu.audio.item.stereoMode.item.stereo" ), mediaPlayer, null));
        actions.add(new StereoModeAction(Resource.resource("menu.audio.item.stereoMode.item.left"   ), mediaPlayer, null));
        actions.add(new StereoModeAction(Resource.resource("menu.audio.item.stereoMode.item.right"  ), mediaPlayer, null));
        actions.add(new StereoModeAction(Resource.resource("menu.audio.item.stereoMode.item.reverse"), mediaPlayer, null));
        return ImmutableList.copyOf(actions);
    }

    private List<Action> newAudioControlActions(MediaPlayer mediaPlayer) {
        List<Action> actions = new ArrayList<>();
        actions.add(new VolumeAction(Resource.resource("menu.audio.item.increaseVolume"), mediaPlayer,  10));
        actions.add(new VolumeAction(Resource.resource("menu.audio.item.decreaseVolume"), mediaPlayer, -10));
    ...
    private List<Action> newVideoZoomActions(MediaPlayer mediaPlayer) {
        List<Action> actions = new ArrayList<>();
        actions.add(new ZoomAction(Resource.resource("menu.video.item.zoom.item.quarter" ), mediaPlayer, 0.25f));
        actions.add(new ZoomAction(Resource.resource("menu.video.item.zoom.item.half"    ), mediaPlayer, 0.50f));
        actions.add(new ZoomAction(Resource.resource("menu.video.item.zoom.item.original"), mediaPlayer, 1.00f));
        actions.add(new ZoomAction(Resource.resource("menu.video.item.zoom.item.double"  ), mediaPlayer, 2.00f));
    ...
    private List<Action> newVideoAspectRatioActions(MediaPlayer mediaPlayer) {
        List<Action> actions = new ArrayList<>();
        actions.add(new AspectRatioAction(Resource.resource("menu.video.item.aspectRatio.item.default"), mediaPlayer, null));
    ...
    private List<Action> newVideoCropActions(MediaPlayer mediaPlayer) {
        List<Action> actions = new ArrayList<>();
        actions.add(new CropAction(Resource.resource("menu.video.item.crop.item.default"), mediaPlayer, null));
